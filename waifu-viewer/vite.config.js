...
function versionSync(){
  // robust ROOT: from this file's location, not process.cwd()
  ...
  function gen(){
    // Prefer canonical root version.json (generated by scripts/version.mjs) to avoid churn.
    // Only generate fresh if file missing or unreadable.
    try{
      if(fs.existsSync(VERSION_JSON)){
        ...
        if(j && typeof j.version === 'string' && j.buildTime){
          // ensure 10-field completeness for older files
    ...
    const p = parse(raw)
    const version = format(p)
    const data = { version, build, commit, branch, dirty, buildTime: new Date().toISOString(), continent:p.continent, versionNumber:`${p.major}.${p.minor}.${p.patch}`, revision:String(p.revision).padStart(2,'0'), npmVersion:npmVersion(p) }
    return data
  }
  function writePublic(data){
    // In dev, public/version.json is a cache — just ensure it exists as copy of root.
    // Do not generate new buildTime here; root is canonical.
    try{
      let src = data;
      if(!src && fs.existsSync(VERSION_JSON)){
        try{ src = JSON.parse(fs.readFileSync(VERSION_JSON,'utf8')) }catch{}
      }
      if(src){
        fs.mkdirSync(path.dirname(PUBLIC_JSON),{recursive:true});
        // only write if public missing or content differs (avoid mtime churn)
        try{
          const cur = fs.existsSync(PUBLIC_JSON) ? fs.readFileSync(PUBLIC_JSON,'utf8') : '';
          const next = JSON.stringify(src,null,2)+'\n';
          if(cur !== next) fs.writeFileSync(PUBLIC_JSON, next);
        }catch{ fs.writeFileSync(PUBLIC_JSON, JSON.stringify(src,null,2)+'\n') }
      }
    }catch{}
    try{ if(!fs.existsSync(VERSION_JSON) && data) fs.writeFileSync(VERSION_JSON, JSON.stringify(data,null,2)+'\n') }catch{}
  }
  // ensure on startup — copy root to public, do not churn buildTime
  try{
    if(fs.existsSync(VERSION_JSON)){
      const j = JSON.parse(fs.readFileSync(VERSION_JSON,'utf8'));
      writePublic(j);
  ...
  return {
    ...
    configureServer(server){
      ...
      server.middlewares.use((req,res,next)=>{
        ...
        // /api/version is proxied to backend (see server.proxy) so frontend gets needsRebuild/hasDist.
        // Fallback to local gen only if backend unreachable is handled by fetchVersion -> /version.json
        next()
      })
      const notify=()=>{
        try{
          const d=gen();
          // do not churn public file on every HMR notify; just send event
    ...
    closeBundle(){
      try{
        // Use canonical root version.json for dist, not fresh gen (avoids buildTime drift)
        let d=null;
