...
- **Neural Cloud** — full-screen mesh realm where every uploaded file (images, documents) is a node: images described once via the free vision router (`openrouter/free`, chat model untouched), documents text-extracted; the AI reads contents through tool calls only when needed, plus everyday tools (time, date, calculator)
...
```
waifu-viewer/         Vite + Three/MMD (src/main.js, services/{waifu-client,morph-driver,tools,benchmark,extensions,content,cloud-view,i18n}, settings/, editor/WpkgEditor as on-demand add-on)
...
VERSION / version.json  App version (EU-0.6.0-01) synced to public/version.json + /api/version
```
